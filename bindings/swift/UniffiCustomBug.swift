...
#if canImport(UniffiCustomBugFFI)
import UniffiCustomBugFFI
#endif
...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_UniffiCustomBug_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_UniffiCustomBug_rustbuffer_free(self, $0) }
    }
}
...
public func `testFn`(`typeParam`: CustomType)  {
    try! rustCall() {
    uniffi_uniffi_custom_bug_fn_func_test_fn(
        FfiConverterTypeCustomType.lower(`typeParam`),$0)
}
}
...
private var initializationResult: InitializationResult {
    ...
    let scaffolding_contract_version = ffi_UniffiCustomBug_uniffi_contract_version()
    if bindings_contract_version != scaffolding_contract_version {
        return InitializationResult.contractVersionMismatch
    }
    if (uniffi_uniffi_custom_bug_checksum_func_test_fn() != 13503) {
